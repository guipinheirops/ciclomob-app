...
function todayView(){
 ...
 ${editable?(current?`<section class="card record-complete-card"><div class="record-complete-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><path d="m6.5 12.3 3.5 3.5 7.8-8" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div><span class="label">REGISTRO CONCLUÍDO</span><h3>Observação de hoje salva</h3><p class="muted">Seu registro foi salvo com sucesso e já está disponível no Gráfico.</p></div></section>`:`<section class="card"><form id="recordForm" class="stack">
  ...
  <div class="field"><label>Aparência observada</label><select name="appearance" required><option value="" selected disabled>Selecione</option>${['Sem observação','Opaca','Transparente','Elástica','Outra'].map(x=>`<option value="${x}">${x}</option>`).join('')}</select></div>
...
function calendarView(){
 ...
 return `<section class="card"><div class="calendar-head"><button class="icon-btn mini" data-month="-1">‹</button><h3>${monthLabel}</h3><button class="icon-btn mini" data-month="1">›</button></div><div class="weekdays">${['SEG','TER','QUA','QUI','SEX','SÁB','DOM'].map(x=>`<span>${x}</span>`).join('')}</div><div class="calendar-grid">${cells.join('')}</div><div class="legend"><span><i class="dot record-dot"></i> Registro</span><span><i class="dot bleed-dot"></i> Sangramento</span><span><i class="dot fertile-dot"></i> Período fértil</span></div></section>${estimateCard}`
}
...
function historyView(){
 ...
 const rows=Array.from({length:len},(_,i)=>{
   ...
   return `<article class="mob-graph-row compact ${editable?'is-editable':'is-locked'} ${isToday?'is-today':''} ${fertileRecord?'fertile-record':''}" ${editable?`data-edit="${r.id}" role="button" tabindex="0" aria-label="Editar registro de hoje"`:`data-locked-record="${r.id}" role="button" tabindex="0" aria-label="Registro bloqueado para edição"`}><div class="mob-cycle-day ${fertileRecord?'fertile-cycle-day':''}"><small>Dia</small><strong>${day}</strong></div><span class="mob-graph-stamp ${r.chartStamp||'unclassified'}" title="${esc(meta.label)}"><b>${meta.symbol}</b></span><div class="mob-graph-content"><div class="mob-graph-date-row"><span class="mob-graph-date">${fmtDate(date,{day:'2-digit',month:'2-digit'})}${isToday?' · Hoje':''}</span><span class="record-date-icons">${baby}${heart}</span></div><div class="mob-graph-summary"><span>${esc(summary||'Registro realizado')}</span></div></div></article>`;
 }).join('');
...
function editView(){
 ...
 return `<section class="hero-card edit-hero"><span class="pill">Edição de hoje</span><h2>Atualize o registro</h2><p class="muted">Apenas o registro de hoje pode ser alterado.</p></section><section class="card"><form id="editRecordForm" class="stack">
  ...
  <div class="field"><label>Aparência observada</label><select name="appearance" required>${['Sem observação','Opaca','Transparente','Elástica','Outra'].map(x=>`<option value="${x}" ${String(r.appearance||'').trim()===x?'selected':''}>${x}</option>`).join('')}</select></div>
