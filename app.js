...
function historyView(){
 ...
 const rows=Array.from({length:len},(_,i)=>{
   ...
   return `<article class="mob-graph-row compact ${editable?'is-editable':''} ${isToday?'is-today':''}" ${editable?`data-edit="${r.id}" role="button" tabindex="0" aria-label="Editar registro de hoje"`:''}><div class="mob-cycle-day"><small>Dia</small><strong>${day}</strong></div><span class="mob-graph-stamp ${r.chartStamp||'unclassified'}" title="${esc(meta.label)}"><b>${meta.symbol}</b></span><div class="mob-graph-content"><span class="mob-graph-date">${fmtDate(date,{day:'2-digit',month:'2-digit'})}${isToday?' · Hoje':''}</span><div class="mob-graph-summary"><span>${esc(summary||'Registro realizado')}</span></div></div></article>`;
 }).join('');
 return `<section class="mob-graph-view"><div class="mob-graph-head"><strong>${len} dias configurados</strong><small>${esc(cycleDisplayName(c))}</small></div><div class="notice mob-edit-today-notice">Apenas o registro de hoje pode ser alterado.</div><div class="mob-month-list mob-flat-list compact-cycle-list">${rows}</div><div class="notice mob-graph-notice">Os selos são informados manualmente e não representam interpretação automática de fertilidade.</div></section>`
}
function editView(){
