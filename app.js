...
function renderAuth(){
 ...
 authShell.innerHTML=state.authReset?passwordResetView():authView();
 if(state.authReset){
...
async function initAuth(){document.documentElement.dataset.theme=state.settings.theme;if(localStorage.getItem('cycleseed.demo')==='1'){state.demoMode=true;state.user={email:'demo@cycleseed.local',id:'demo'};await showApp();return}if(!db){renderAuth();return}const {data}=await db.auth.getSession();state.user=data.session?.user||null;if(state.user){if(await hasPaidAccess(state.user.id)){prepareRealUserState(state.user.id);await showApp()}else{await db.auth.signOut();state.user=null;renderAuth()}}else renderAuth();db.auth.onAuthStateChange(async(_event,session)=>{if(session?.user){const changed=state.user?.id!==session.user.id;state.user=session.user;if(changed)prepareRealUserState(state.user.id);if(authShell&&!authShell.classList.contains('hidden'))await showApp()}else if(!state.demoMode){state.user=null;renderAuth()}})}
