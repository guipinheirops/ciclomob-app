...
const lessons = [
  ...
  {id:'pbi',title:'4. Conheça o PBI',time:'6 min',text:'Entenda o que significa Padrão Básico de Infertilidade e por que ele não deve ser marcado sem aprendizado adequado.',sections:[
    ['O que é PBI','PBI significa Padrão Básico de Infertilidade. No app, a marcação é manual e deve ser usada somente quando esse padrão já tiver sido reconhecido pela usuária conforme seu aprendizado do método.'],
  ...
  {id:'review',title:'5. Revise o ciclo completo',time:'7 min',text:'Use o gráfico, o calendário e o resumo para revisar a sequência de observações do ciclo selecionado.',sections:[
    ...
    ['Gráfico','Revise os registros em sequência, observando dia do ciclo, selo, sensação, aparência, sangramento e Pico. O Pico é confirmado retrospectivamente, depois que termina a sensação escorregadia. A partir dessa confirmação, o aplicativo organiza automaticamente +1, +2 e +3.'],
    ['Resumo','Use o resumo para consultar uma visão descritiva das categorias registradas no ciclo selecionado.'],
    ['Confirmação do Pico','O Pico não é marcado antecipadamente. Confirme-o retrospectivamente quando a sensação escorregadia terminar; o aplicativo então organiza +1, +2 e +3.']
  ]},
  {id:'stamps',title:'6. Conheça os selos do gráfico',time:'5 min',text:'Aprenda o significado visual dos selos usados para organizar os registros no Gráfico MOB.',sections:[
    ...
    ['Escolha do selo','O selo é obrigatório no registro diário, mas deve ser escolhido manualmente conforme o que foi observado e aprendido no método.'],
...
function seedRecords(){const today=new Date(),arr=[];for(let i=12;i>=0;i--){const d=new Date(today);d.setDate(today.getDate()-i);arr.push({id:crypto.randomUUID(),date:d.toISOString().slice(0,10),sensation:['Seca','Úmida','Molhada','Escorregadia'][i%4],appearance:['Sem observação','Opaca','Transparente','Elástica'][i%4],bleeding:i>=8&&i<=12?['Leve','Moderado','Moderado','Leve','Leve'][12-i]:'Não',pbi:i===3,intercourse:false,chartStamp:(i>=8&&i<=12?'red':(i%4===0?'green':i===3?'yellow':'white')),peakMarker:(i===1?'peak':''),notes:i===1?'Observação feita à noite.':'',createdAt:Date.now()})}return arr}
...
initialRecords.forEach(r=>{if(!r.cycleId)r.cycleId=initialActiveCycleId;if(typeof r.intercourse!=='boolean')r.intercourse=false});
...
function canEditRecord(r){const c=state.cycles.find(c=>c.id===r?.cycleId);return Boolean(c&&(c.accessRole||'owner')==='owner'&&!c.endDate&&r?.date===TODAY)}
...
function mobObservationSignature(r){
 if(!r)return '';
 return [
  String(r.sensation||'').trim().toLowerCase(),
  String(r.appearance||'').trim().toLowerCase(),
...
function calendarView(){
 ...
 return `<section class="card"><div class="calendar-head"><button class="icon-btn mini" data-month="-1">‹</button><h3>${monthLabel}</h3><button class="icon-btn mini" data-month="1">›</button></div><div class="weekdays">${['SEG','TER','QUA','QUI','SEX','SÁB','DOM'].map(x=>`<span>${x}</span>`).join('')}</div><div class="calendar-grid">${cells.join('')}</div><div class="legend"><span><i class="dot bleed-dot"></i> Sangramento</span><span><i class="dot fertile-dot"></i> Fértil</span><span><i class="dot liberated-dot"></i> Liberado</span></div></section>${estimateCard}`
}
...
function mobChartView(){
 ...
 const rows=sorted.map(r=>{
   ...
   return `<article class="mob-list-row ${cycleClosed?'cycle-closed-record':''}">
    <div class="mob-list-date">${fmtDate(r.date,{day:'2-digit',month:'2-digit',year:'numeric'})}${r.intercourse?`<span class="record-heart inline-heart" title="Relação informada" aria-label="Relação informada"><svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 20.2 4.4 12.9C.4 9.1 2.7 3 7.6 3c2 0 3.5 1 4.4 2.3C12.9 4 14.4 3 16.4 3c4.9 0 7.2 6.1 3.2 9.9L12 20.2Z"/></svg></span>`:''}</div>
...
function historyView(){
 const c=activeCycle(),len=Math.max(1,Number(c?.cycleLength)||cycleLength());
 const records=new Map(activeRecords().map(r=>[r.date,r]));
 const rows=Array.from({length:len},(_,i)=>{
   const day=i+1,date=addDays(c.startDate,i),r=records.get(date),isToday=date===TODAY,isFuture=date>TODAY;
   if(!r){
...
function editView(){
 ...
 if(!r)return `<section class="card"><h3>Registro não encontrado</h3><p class="muted">O registro pode ter sido removido.</p><button class="secondary" data-go="history">Voltar ao gráfico</button></section>`;
 if(!canEditRecord(r))return `<section class="card"><h3>Registro bloqueado</h3><p class="muted">Apenas o registro de hoje pode ser alterado.</p><button class="secondary" data-go="cycle">Voltar ao gráfico</button></section>`;
 return `<section class="hero-card edit-hero"><span class="pill">Edição de hoje</span><h2>Atualize o registro</h2><p class="muted">Apenas o registro de hoje pode ser alterado.</p></section><section class="card"><form id="editRecordForm" class="stack">
  <div class="field"><label>Data</label><input name="date" type="date" value="${TODAY}" readonly required></div>
...
function reminderSettingsCard(){
 ...
 return `<section class="hero-card reminder-hero branded-feature-card"><span class="pill">Lembretes</span><h2>Não deixe o registro do dia passar</h2><p class="muted">Escolha um horário e, quando permitido pelo aparelho, receba um aviso mesmo com o aplicativo fechado.</p></section><section class="card"><div class="stack"><div class="feature-status"><i class="status-dot ${pushActive?'ok':'warn'}"></i><div><b>${pushActive?'Notificações ativadas':'Notificações não ativadas'}</b><small>${pushSupported?'Compatível com este navegador.':'Este navegador não oferece notificações.'}</small></div></div><div class="profile-row"><div><strong>Lembrete diário</strong><div class="label">Lembrar de registrar o dia</div></div><button class="switch ${state.settings.reminder?'on':''}" data-setting="reminder"></button></div><div class="reminder-time-card"><div class="reminder-time-copy"><span class="reminder-clock-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="8.2" stroke="currentColor" stroke-width="1.8"/><path d="M12 7.8v4.7l3.2 1.9" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></span><div><strong>Horário do lembrete</strong></div></div><div class="reminder-time-input-wrap"><input id="reminderTime" type="time" value="${state.settings.reminderTime}" aria-label="Horário do lembrete"></div></div><div class="action-grid"><button id="enablePushBtn" class="primary">${pushActive?'Atualizar notificações':'Ativar notificações'}</button><button id="testPushBtn" class="secondary">Enviar teste</button></div><p class="subtle">Ative as notificações e mantenha o horário desejado. O lembrete será enviado mesmo com o aplicativo fechado, quando permitido pelo navegador.</p></div></section>`
}
...
function profileView(){
 ...
 const sessionCard='';
 return `${setupCard}<section class="card account-card account-card-top"><div class="account-avatar">${esc((state.settings.profileName||state.user?.email||'D').slice(0,1).toUpperCase())}</div><div class="account-card-copy"><strong>${esc(state.settings.profileName||who)}</strong><small>${state.demoMode?'Dados apenas neste dispositivo':esc(state.user?.email||'Sessão protegida')}</small></div><button id="logoutBtn" class="secondary account-logout-btn">${state.demoMode?'Encerrar':'Sair'}</button></section>
 ${cycleCard}
...
async function saveRecordCloud(r){const rc=state.cycles.find(c=>c.id===r.cycleId);if(!db||!state.user||state.demoMode||rc?.accessRole!=='owner')return;const payload={id:r.id,cycle_id:r.cycleId,date:r.date,sensation:r.sensation,appearance:r.appearance,bleeding:r.bleeding,pbi:Boolean(r.pbi),intercourse:Boolean(r.intercourse),chart_stamp:r.chartStamp||null,peak_marker:r.peakMarker||null,notes:r.notes||'',recorded_by:state.user.id};const {error}=await db.from('cycle_records').upsert(payload,{onConflict:'id'});if(error)console.warn('Record sync',error)}
...
async function syncCloudData(){if(!db||!state.user||state.demoMode)return;state.cloudLoading=true;try{
 ...
 if(cloudCycles?.length){
  ...
  const ids=state.cycles.map(c=>c.id);const {data:cloudRecords,error:rErr}=await db.from('cycle_records').select('id,cycle_id,date,sensation,appearance,bleeding,pbi,intercourse,chart_stamp,peak_marker,notes,created_at,updated_at').in('cycle_id',ids).order('date',{ascending:true});if(rErr)throw rErr;
  state.records=(cloudRecords||[]).map(r=>({id:r.id,cycleId:r.cycle_id,date:r.date,sensation:r.sensation,appearance:r.appearance,bleeding:r.bleeding,pbi:r.pbi,intercourse:Boolean(r.intercourse),chartStamp:r.chart_stamp||'',peakMarker:r.peak_marker||'',notes:r.notes,createdAt:r.created_at,updatedAt:r.updated_at}));
...
function generatePDF(){const jsPDF=window.jspdf?.jsPDF;if(!jsPDF){toast('Biblioteca de PDF ainda não carregou');return}const doc=new jsPDF({unit:'mm',format:'a4'}),pageW=210,margin=14;let y=16;const line=(txt,size=9,bold=false)=>{doc.setFont('helvetica',bold?'bold':'normal');doc.setFontSize(size);const parts=doc.splitTextToSize(cleanPdfText(txt),pageW-margin*2);if(y+parts.length*4.8>282){doc.addPage();y=16}doc.text(parts,margin,y);y+=parts.length*4.8};
 ...
 [...activeRecords()].sort((a,b)=>a.date.localeCompare(b.date)).forEach(r=>{line(`${fmtDate(r.date,{day:'2-digit',month:'2-digit',year:'numeric'})}  |  ${r.sensation}  |  ${r.appearance}  |  Sangramento: ${r.bleeding}${r.pbi?'  |  PBI':''}${r.intercourse?'  |  Relação: Sim':''}  |  Selo: ${mobStampMeta(r).label}${peakLabel(r.peakMarker)?'  |  '+peakLabel(r.peakMarker):''}`,8.5,true);if(r.notes)line('Notas: '+r.notes,8);y+=1});
...
function render(){const titles={today:'Hoje',cycle:'Meu ciclo',history:'Gráfico',edit:'Editar registro',learn:'Aprender',lesson:(lessons.find(l=>l.id===state.lessonId)?.title||'Módulo'),support:'Apoio',profile:'Perfil',reminders:'Lembretes'};title.textContent=titles[state.route]||'Ciclo MOB';const isSubView=state.route==='lesson'||state.route==='edit';backBtn.classList.toggle('hidden',!isSubView);backBtn.onclick=state.route==='lesson'?()=>route('learn'):state.route==='edit'?()=>{if(state.editReturn==='cycle')state.cycleTab='chart';route(state.editReturn||'cycle')}:null;app.innerHTML=({today:todayView,cycle:cycleView,history:historyView,edit:editView,learn:learnView,lesson:lessonView,support:supportView,profile:profileView,reminders:remindersView}[state.route])();const navRoute=state.route==='lesson'?'learn':state.route==='edit'&&state.editReturn==='cycle'?'cycle':state.route;nav.forEach(n=>n.classList.toggle('active',n.dataset.route===navRoute));if(state.route==='cycle'&&state.cycleTab==='chart'){requestAnimationFrame(()=>{const currentRow=document.querySelector('[data-current-day="1"]');if(currentRow)currentRow.scrollIntoView({block:'center',behavior:'auto'})})}updateHeaderActions();bind()}
function bind(){
