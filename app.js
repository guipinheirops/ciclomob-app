...
function calendarView(){
 ...
 return `<section class="card"><div class="calendar-head"><button class="icon-btn mini" data-month="-1">‹</button><h3>${monthLabel}</h3><button class="icon-btn mini" data-month="1">›</button></div><div class="weekdays">${['SEG','TER','QUA','QUI','SEX','SÁB','DOM'].map(x=>`<span>${x}</span>`).join('')}</div><div class="calendar-grid">${cells.join('')}</div><div class="legend"><span><i class="dot bleed-dot"></i> Sangramento</span><span><i class="dot fertile-dot"></i> Fértil</span><span><i class="dot liberated-dot"></i> Liberado</span><span><i class="dot stamp-green-dot"></i> Selo verde</span><span><i class="dot stamp-white-dot"></i> Selo branco</span><span><i class="dot stamp-yellow-dot"></i> Selo amarelo</span></div></section>${estimateCard}`
}
