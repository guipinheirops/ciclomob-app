...
function mobFertilityStatus(c){
 if(!c||c.endDate)return {key:'observing',active:false,label:'Em observação',detail:'Ciclo encerrado ou sem dados suficientes'};
...
function calendarView(){
 ...
 for(let d=1;d<=last.getDate();d++){const date=new Date(y,m,d,12).toISOString().slice(0,10),rec=activeRecords().find(r=>r.date===date),cd=cycleDay(date);let cls='calendar-day';if(date===TODAY)cls+=' today';if(rec)cls+=' has-record';const hasBleeding=Boolean(rec&&rec.bleeding&&rec.bleeding!=='Não');if(hasBleeding)cls+=' bleeding';cells.push(`<button class="${cls}" data-date="${date}"><span>${d}</span>${cd?`<small>D${cd}</small>`:''}${rec?'<i></i>':''}</button>`)}
