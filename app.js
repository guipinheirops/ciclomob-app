...
function calendarView(){
 ...
 for(let d=1;d<=last.getDate();d++){const date=new Date(y,m,d,12).toISOString().slice(0,10),rec=activeRecords().find(r=>r.date===date),cd=cycleDay(date);let cls='calendar-day';if(date===TODAY)cls+=' today';if(rec)cls+=' has-record';const hasBleeding=Boolean(rec&&rec.bleeding&&rec.bleeding!=='Não');if(hasBleeding)cls+=' bleeding';const inCycle=Boolean(cd&&date<=TODAY),fertileDay=inCycle&&mobFertilityStatusAt(activeCycle(),date).active;if(fertileDay)cls+=' mob-fertile-day';cells.push(`<button class="${cls}" data-date="${date}"><span>${d}</span>${cd?`<small>D${cd}</small>`:''}${rec?'<i></i>':''}${fertileDay?'<b class="fertile-day-mark" aria-label="Período fértil"></b>':''}</button>`)}
...
function historyView(){
 ...
 const rows=Array.from({length:len},(_,i)=>{
   ...
   const meta=mobStampMeta(r),peak=peakLabel(r.peakMarker),editable=canEditRecord(r);
   ...
   return `<article class="mob-graph-row compact ${editable?'is-editable':'is-locked'} ${isToday?'is-today':''}" ${editable?`data-edit="${r.id}" role="button" tabindex="0" aria-label="Editar registro de hoje"`:`data-locked-record="${r.id}" role="button" tabindex="0" aria-label="Registro bloqueado para edição"`}><div class="mob-cycle-day"><small>Dia</small><strong>${day}</strong></div><span class="mob-graph-stamp ${r.chartStamp||'unclassified'}" title="${esc(meta.label)}"><b>${meta.symbol}</b></span><div class="mob-graph-content"><span class="mob-graph-date">${fmtDate(date,{day:'2-digit',month:'2-digit'})}${isToday?' · Hoje':''}</span><div class="mob-graph-summary"><span>${esc(summary||'Registro realizado')}</span></div></div></article>`;
 }).join('');
